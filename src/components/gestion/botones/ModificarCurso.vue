    ...
    <v-container fluid>
        ...
        <v-dialog v-model="dialog" max-width="1280">
            <v-card>
                ...
                <v-card-text>
                    <v-form @submit.prevent="modificarCurso()">
                        <v-container>
                            <v-row justify="center">
                                <!-- mostramos una tabla con los cursos registrados para saber cual modificar -->
                                <v-data-table
                                        :headers="headers"
                                        :items="desserts"
                                        :items-per-page="5"
                                ...
                                ></v-data-table>
                                <v-col cols="12" lg="10">
                                    <!--  aqui seleccionamos el curso que hay que modificar usando el cod del curso-->
                                    <v-select
                                            v-model="cursoViejo"
                                            :items="codigosCursos"
                                            label="Seleccione el curso a modificar"
                                            required
                                    ></v-select>
...
<script>
    ...
    export default {
        ...
        data: ()=>({
            cursoViejo: '',//contiene el cogido del curdso que se va  a modificar
            codigosCursos: [],//contiene los cofigos de todos los cursos para ser utilizado en el reemplazamiento de datos
        ...
        methods:{
            ...
            * esta funcion carga los datos introducidps en el formulario en la base de datos
            ...
            async getAPI(){
                ...
                let funcionDatosCursos = new FormData;
                funcionDatosCursos.append('func','mostrarCursos');
                let datosCursos = await axios.post(this.APIExterna,funcionDatosCursos);
                this.desserts = await datosCursos.data;
                let funcionCodigosCursos = new FormData;
                funcionCodigosCursos.append('func','mostrarCodigosCursos');
                let datosCodigosCursos = await axios.post(this.APIExterna,funcionCodigosCursos);
                this.codigosCursos = datosCodigosCursos.data;
            },
            /*
            ...
            */
            async modificarCurso(){
                let funcion = new FormData;
                funcion.append('func','modificarCurso');
                funcion.append('cursoViejo',this.cursoViejo);
        ...
        computed: {
            ...
            cursoCorrecto(){
                return this.nombreEmpresa !== "" && this.dniConductor !== "" && this.dniInstructor !== "" && this.dniAyudante !== ""
                    ...
                    && this.trayecto !== "" && !isNaN(this.kmLLegada) && !isNaN(this.kmIda) && this.cursoViejo !== "";
